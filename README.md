...
也可以直接下载 Release 里的 zip 解压到 skills 目录。
...
本 Skill 整合了两套**互补**的数学建模工作流，各自的强项恰好补上对方的短板：

| 来源 | 强项 | 互补关系 |
|---|---|---|
| [LEEHAHAHAHA/math-modeling-skill](https://github.com/LEEHAHAHAHA/math-modeling-skill) | 从读题到交付的**完整流程能力**与全部资源（模板、脚本、子技能） | 解决「**怎么把活干完**」——提供广度 |
| 门控式竞赛工作流 | 基线对比、独立数据挖掘、独立测试集、文献核验、数值冻结等**硬约束** | 解决「**怎么保证干对**」——提供深度 |

一句话：**广度 + 深度**。只有流程能力，容易跑完却跑偏；只有门禁，要求很严却缺少落地的手段。两者合起来，才既能跑完、又能跑对。

本 Skill 中来自 [LEEHAHAHAHA/math-modeling-skill](https://github.com/LEEHAHAHAHA/math-modeling-skill) 的内容：
...
本 Skill 新增的部分：独立数据挖掘章、独立测试集检验、基线对比门、文献核验门、数值冻结门，以及 `check_gates.py` 门禁自检脚本。全部以 MIT 许可证开源。

感谢原作者 **LEEHAHAHAHA** 及所有贡献者。如果本项目对你有帮助，也请一并给原项目点个 star。
